...
RSpec.describe Church, type: :model do
  ...
  it 'メールアドレスが入力されていればaタグで表示すること' do
    ...
    expect(church.display_email).to eq "<a href='mailto:***REMOVED***{church.email}'>メールはこちら</a>"
  end
  ...
  it 'ホームページが入力されていれば表示すること' do
    ...
    expect(church.display_url).to eq "<a href=***REMOVED***{church.url} target='_blank'>ホームページはこちら</a>"
  end
